...
app.post('/login', (req, res) => {
  ...
  db.get("SELECT * FROM users WHERE username = ?", [username], async (err, user) => {
    if (err || !user) return res.status(401).json({ message: "Login lub Hasło niepoprawny" });
    
    if (user.is_blocked) return res.status(403).json({ message: "Konto zablokowane" });

    const validPassword = await bcrypt.compare(password, user.password);
    if (!validPassword) return res.status(401).json({ message: "Login lub Hasło niepoprawny" });
  ...
  db.get("SELECT * FROM settings", (err, settings) => {
    ...
    db.get("SELECT * FROM users WHERE id = ?", [userId], async (err, user) => {
      ...
      db.run(`UPDATE users SET password = ?, must_change_password = 0, last_password_change = CURRENT_TIMESTAMP, previous_passwords = ? WHERE id = ?`,
        [newHash, JSON.stringify(prevPasswords), userId],
        (err) => {
...
app.put('/settings', verifyToken, (req, res) => {
  ...
  db.run("UPDATE settings SET min_length=?, require_digit=?, require_special_char=?, require_mixed_case=?, validity_days=?",
    [min_length, require_digit, require_special_char, require_mixed_case, validity_days],
    () => res.json({ success: true })
  );
});
